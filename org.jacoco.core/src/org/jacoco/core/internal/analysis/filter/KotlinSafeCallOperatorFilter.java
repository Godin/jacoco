		...
		for (final ArrayList<JumpInsnNode> chain : findChains(methodNode)) {
			...
			final Iterable<Collection<IFilterOutput.InstructionBranch>> replacements = Arrays.<Collection<IFilterOutput.InstructionBranch>> asList(
					Collections.singletonList(
							new IFilterOutput.InstructionBranch(lastJump, 0)),
					Collections.singletonList(
							new IFilterOutput.InstructionBranch(nullTarget,
									0)));

			// final ArrayList<IFilterOutput.InstructionBranch> a = new
			// ArrayList<IFilterOutput.InstructionBranch>();
			// for (final AbstractInsnNode ifNullInstruction : chain) {
			// a.add(new IFilterOutput.InstructionBranch(ifNullInstruction,
			// 1));
			// }
