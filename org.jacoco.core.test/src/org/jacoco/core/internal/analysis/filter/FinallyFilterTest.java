...
public class FinallyFilterTest implements IFilterOutput {
	...
	/**
	 * TODO not in {@link org.jacoco.core.test.filter.FinallyTest}, can't be
	 * handled by {@link FinallyPreviousFilter}.
	 *
	 * <pre>
	 *   try {
	 *     ...
	 *     if (...) {
	 *       ...
	 *       return; // or break or continue of loop outside of try-block, but not throw
	 *     }
	 *   } finally {
	 *     ...
	 *   }
	...
	public void javac() throws Exception {
		...
		m.visitInsn(Opcodes.NOP);
		Label l3 = new Label();
		m.visitJumpInsn(Opcodes.IFEQ, l3);
		m.visitInsn(Opcodes.NOP);
		m.visitLabel(tryEnd);
