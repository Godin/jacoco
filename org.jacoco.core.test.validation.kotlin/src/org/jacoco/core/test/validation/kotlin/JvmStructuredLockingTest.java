...
public class JvmStructuredLockingTest {
	...
	public void unreachable_handler() throws Exception {
		...
		MonitorsAnalyzer.EVALUATE_IDEA = true;
		final byte[] instrumentedBytes;
		try {
			instrumentedBytes = instrument(classBytes);
			System.out.println(AsmUtils.toString(instrumentedBytes));
		} finally {
			MonitorsAnalyzer.EVALUATE_IDEA = false;
		}
		Assert.assertEquals("", new JvmProcess().execute("Main", classBytes));
		Assert.assertEquals("",
				new JvmProcess().execute("Main", instrumentedBytes));
	}
	...
	private void createRepro() {
		...
		// Note try-catch start and end labels are modified to be before probe,
		// otherwise probe is before label
		Label label0 = new Label();
		Label label1 = new Label();
		Label label2 = new Label();
